...
function DetailQuestionnaire() {
  ...
  const [questionnaireSite, setQuestionnaireSite] = useState(null); // Nouveau state pour les données du site
  const theme = useTheme();
  const primaryColor = theme.palette.primary.main;

  useEffect(() => {
    // Charger les données du questionnaire principal
    ...
    // Charger les données du questionnaire du site
    const fetchQuestionnaireSite = async () => {
      try {
        const response = await fetch(`http://localhost:5000/api/questionnaire_site`);
        const datasite = await response.json();
        const matchedsite = datasite.find(q => slugify(q.titre) === titre);
        setQuestionnaireSite(matchedsite);
      } catch (error) {
      ...
      >
        <Grid container spacing={3} justifyContent="center">
          <Grid item xs={12} md={8}>
            ...
            {/* Affichage des questionnaires de site */}
            <Divider sx={{ my: 3, backgroundColor: primaryColor, opacity: 0.7 }} />
            <Typography
              variant="h4"
              fontWeight="bold"
              ...
              sx={{ color: primaryColor }}
            >
              Questionnaire du Site : {questionnaireSite?.titre || "Inconnu"}
            </Typography>

            {questionnaireSite?.sections?.map((section, idx) => (
              <Accordion
                key={idx}
                elevation={4}
                sx={{
              ...
              >
                ...
                >
                  ...
                  >
                    <ListAltIcon sx={{ mr: 2, fontSize: 24, color: primaryColor }} />
                    Site Section {idx + 1}: {section.titre}
                  </Typography>
                </AccordionSummary>
                <AccordionDetails sx={{ p: 3, backgroundColor: theme.palette.grey[50] }}>
                  {section.questions?.map((question, qIdx) => (
