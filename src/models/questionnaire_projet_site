...
// Réutilisation des sous-schemas :
...
// 🔶 Schéma principal
const QuestionnaireProjetSiteSchema = new mongoose.Schema({
  siteId: {
    type: mongoose.Schema.Types.ObjectId,
    required: true,
    ref: 'Site' // nom du modèle du site si tu en as un
  },
  titre: { type: String, required: true },
  ...
  projet: { type: String },
  index: { type: Number },
  analyse: { type: AnalyseSchema, default: null }
});

module.exports = mongoose.model('questionnaire_projet_site', QuestionnaireProjetSiteSchema);
